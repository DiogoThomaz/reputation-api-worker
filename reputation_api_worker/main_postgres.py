...
def run_playstore(db: PostgresClient, call_check_id: callable = _check_id_exist):
    ...
    for row in empresas:
        ...
        logger.info(
            "[Play Store] Coletando | empresa=%s app_id=%s",
            empresa_nome,
            playstore_id,
        )
        ...
        for review in reviews:
            ...
            if exist:
                logger.debug(
                    "[Play Store] Review ja existe, pulando | id=%s empresa=%s",
                    review["id"],
                    empresa_nome,
                )
                pulados += 1
            ...
            db.execute(
                query=(
                    ...
                    "(source, empresa, data, review, quantidade_estrelas, external_id) "
                    "values (%s, %s, %s, %s, %s, %s)"
                ),
                params=(
                    "playstore",
            ...
            inseridos += 1
            logger.debug(
                "[Play Store] Review inserido | id=%s empresa=%s",
                review["id"],
                empresa_nome,
            )
...
def run_reclame_aqui(db: PostgresClient, call_check_id: callable = _check_id_exist):
    ...
    for row in empresas:
        ...
        logger.info(
            "[Reclame Aqui] Coletando | empresa=%s slug=%s",
            empresa_nome,
            reclame_aqui_id,
        )
        ...
        with ReclameAquiScraper(max_pages=50) as scrapper:
            ...
            for review in reviews:
                ...
                if exist:
                    logger.debug(
                        "[Reclame Aqui] Review ja existe, pulando | id=%s empresa=%s",
                        review["id"],
                        empresa_nome,
                    )
                    pulados += 1
                ...
                db.execute(
                    query=(
                        "insert into reviews_v1 "
                        "(source, empresa, data, review, titulo, local, external_id) "
                ...
                inseridos += 1
                logger.debug(
                    "[Reclame Aqui] Reclamacao inserida | id=%s empresa=%s titulo=%s",
                    review["id"],
                    empresa_nome,
                    review.get("titulo", "")[:60],
                )
